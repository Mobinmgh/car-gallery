...
**Placement:** full-bleed background, car three-quarter angle
**Prompt:** [exact prompt]
**Aspect ratio:** 16:9# AGENTS.md — MGHBranding Portfolio: Persian Luxury BMW Gallery Landing
...
## Workflow: builder session → reviewer session

This build happens across two Claude Code sessions. The builder session executes step by step. A separate session acts as lead agent and reviews the builder's work before the next step proceeds.
...
- After each section, commit to git with a message naming the section (`feat: hero section`, not `updates`).
- After each section, take a screenshot (use whatever headless browser tool is available — Playwright, or your environment's own screenshot capability) and save it to `/screenshots/section-N.png`.
- Append one line to `CHANGELOG.md` per section: what was built, any deviation from this spec and why.
- Do not skip ahead. Do not batch multiple sections into one commit.

**Reviewer session rules:**
- Read this AGENTS.md, `CHANGELOG.md`, and the latest screenshot before saying anything about the work.
- Check each shipped section against its spec above: does it use the locked token system, does it respect the motion priority order, does it stay in RTL, does the copy avoid generic superlatives.
- Flag drift specifically — name the section, name the deviation, name the fix. Don't say "looks good" without checking against the spec.
- If a section is off, write the fix directly into `CHANGELOG.md` as a review note before the builder session continues, so the correction is on record, not just spoken and forgotten.
- Do not rebuild from scratch. Correct in place.
